...
function buttons_getLikeButton() {
  return buttons_getButtons().children[0].tagName ===
    'YTD-SEGMENTED-LIKE-DISLIKE-BUTTON-RENDERER'
      ? querySelector(extConfig.selectors.buttons.likeButton.segmented) ?? querySelector(extConfig.selectors.buttons.likeButton.segmentedGetButtons, buttons_getButtons())
...
function buttons_getDislikeButton() {
  return buttons_getButtons().children[0].tagName ===
  'YTD-SEGMENTED-LIKE-DISLIKE-BUTTON-RENDERER'
    ? querySelector(extConfig.selectors.buttons.dislikeButton.segmented) ?? querySelector(extConfig.selectors.buttons.dislikeButton.segmentedGetButtons, buttons_getButtons())
    : isShorts() ? querySelector(['#dislike-button'], buttons_getButtons()) : querySelector(extConfig.selectors.buttons.dislikeButton.notSegmented, buttons_getButtons())
}

function createDislikeTextContainer() {
  const textNodeClone =  (buttons_getLikeButton().querySelector('.yt-spec-button-shape-next__button-text-content') || (buttons_getLikeButton().querySelector("button > div[class*='cbox']") || (buttons_getLikeButton().querySelector('div > span[role="text"]') || document.querySelector('button > div.yt-spec-button-shape-next__button-text-content > span[role="text"]')).parentNode)).cloneNode(true);
...
function buttons_checkForSignInButton() {
  if (
...
const state_LIKED_STATE = "LIKED_STATE";
const state_DISLIKED_STATE = "DISLIKED_STATE";
const state_NEUTRAL_STATE = "NEUTRAL_STATE";
...
let state_storedData = {
  likes: 0,
  dislikes: 0,
  previousState: state_NEUTRAL_STATE,
};
...
if (state_isShorts() && !shortsObserver) {
  ...
  shortsObserver = utils_createObserver({
    attributes: true
  }, (mutationList) => {
...
function getState(storedData) {
  if (isVideoLiked()) {
    return { current: state_LIKED_STATE, previous: storedData.previousState };
  }
  if (isVideoDisliked()) {
    return { current: state_DISLIKED_STATE, previous: storedData.previousState };
  }
  return { current: state_NEUTRAL_STATE, previous: storedData.previousState };
}

//---   Sets The Likes And Dislikes Values   ---//
function state_setLikes(likesCount) {
  cLog(`SET likes ${likesCount}`)
  getLikeTextContainer().innerText = likesCount;
}

function state_setDislikes(dislikesCount) {
...
function state_getLikeCountFromButton() {
  try {
    if (state_isShorts()) {
...
function processResponse(response, storedData) {
  ...
  state_setDislikes(formattedDislike);
  if (state_extConfig.numberDisplayReformatLikes === true) {
    const nativeLikes = state_getLikeCountFromButton();
    if (nativeLikes !== false) {
      state_setLikes(numberFormat(nativeLikes));
    }
  }
  storedData.dislikes = parseInt(response.dislikes);
  storedData.likes = state_getLikeCountFromButton() || parseInt(response.likes);
  createRateBar(storedData.likes, storedData.dislikes);
...
async function setState(storedData) {
  ...
  let likeCount = state_getLikeCountFromButton() || null;
...
async function setInitialState () {
  await setState(state_storedData);
}
...
function popup_initializeNumberDisplayFormat() {
  chrome.storage.sync.get(["numberDisplayFormat"], (res) => {
    popup_handleNumberDisplayFormatChangeEvent(res.numberDisplayFormat);
  });
...
function popup_initializeNumberDisplayReformatLikes() {
  chrome.storage.sync.get(["numberDisplayReformatLikes"], (res) => {
    popup_handleNumberDisplayReformatLikesChangeEvent(res.numberDisplayReformatLikes);
  });
}

chrome.storage.onChanged.addListener(popup_storageChangeHandler);

function popup_storageChangeHandler(changes, area) {
  if (changes.disableVoteSubmission !== undefined) {
    popup_handleDisableVoteSubmissionChangeEvent(
      changes.disableVoteSubmission.newValue
    );
  }
  if (changes.coloredThumbs !== undefined) {
    popup_handleColoredThumbsChangeEvent(changes.coloredThumbs.newValue);
  }
  if (changes.coloredBar !== undefined) {
    popup_handleColoredBarChangeEvent(changes.coloredBar.newValue);
  }
  if (changes.colorTheme !== undefined) {
    popup_handleColorThemeChangeEvent(changes.colorTheme.newValue);
  }
  if (changes.numberDisplayFormat !== undefined) {
    popup_handleNumberDisplayFormatChangeEvent(changes.numberDisplayFormat.newValue);
  }
  ...
  if (changes.numberDisplayReformatLikes !== undefined) {
    popup_handleNumberDisplayReformatLikesChangeEvent(
      changes.numberDisplayReformatLikes.newValue
    );
  }
}

function popup_handleDisableVoteSubmissionChangeEvent(value) {
  config.disableVoteSubmission = value;
  document.getElementById("disable_vote_submission").checked = value;
}

function popup_handleColoredThumbsChangeEvent(value) {
  config.coloredThumbs = value;
  document.getElementById("colored_thumbs").checked = value;
}

function popup_handleColoredBarChangeEvent(value) {
  config.coloredBar = value;
  document.getElementById("colored_bar").checked = value;
}

function popup_handleColorThemeChangeEvent(value) {
  if (!value) {
    value = "classic";
  }
...
function popup_handleNumberDisplayFormatChangeEvent(value) {
...
function popup_handleNumberDisplayReformatLikesChangeEvent(value) {
